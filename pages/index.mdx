# 介绍 Introduction

欢迎来到MakiFBT！
Welcome to MakiFBT!

## 什么是MakiFBT？ What is MakiFBT?

MakiFBT是一套无穿戴的全身动作捕捉方案，旨在带给玩家更加**舒适**、**便捷**、**轻量化**的全身动作追踪游戏体验。

MakiFBT is a tracker-less full-body motion capture solution, aiming to bring players more **comfortable**, **convenient**, **lightweight** full-body motion tracking gaming experience.

MakiFBT采用了边缘设备，通过采集图像，计算还原人体3D关键点。您无需再佩戴那些臃肿的追踪器也可以享受同样的追踪效果！

MakiFBT uses edge devices to calculate and restore 3D key points of the human body by capturing multi-angle images of player. You can enjoy the same tracking effect without wearing those bulky trackers!

## 硬件需求 Requirements

一台能够运行SteamVR的电脑(需要 Windows10)

A computer capable of running SteamVR (requires Windows 10).

两部Android手机(Android9以上，需要有后置摄像头)

Two Android phones (Android9 and above, Requires a rear camera).

一台无线路由器(电脑与手机均需要连接到路由器上，推荐电脑使用有线方式连接)

A wireless router (both the computer and the mobile phone need to be connected to the router, it is recommended to use a wired connection for the computer).

一张打印有指定图案的A4纸

A piece of A4 paper with a specified pattern printed on it.

这一些就足够了！

That's enough!